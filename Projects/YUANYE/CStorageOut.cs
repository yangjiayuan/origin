...
namespace YUANYE
{
    ...
    public class CStorageOut : ToolDetailForm
    {
        ...
        void grid_AfterCellUpdate(object sender, Infragistics.Win.UltraWinGrid.CellEventArgs e)
        {
            ...
            switch (e.Cell.Column.Key)
            {
                ...
                case "CTN":
                    ...
                    if ((e.Cell.Row.Cells["CTN"].Value != DBNull.Value) && (e.Cell.Value != DBNull.Value))
                    {
        ...
        public override Form GetForm(CRightItem right, Form mdiForm)
        {
            ...
            {
                Msg.Warning(string.Format("出货通知单加载失败！%n {0}", ex.Message));
            }
            return null;
        }
         ...
         public override bool Check(Guid ID, System.Data.SqlClient.SqlConnection conn, System.Data.SqlClient.SqlTransaction sqlTran, DataRow mainRow)
        {
            ...
            try
            {
                ...
                string SQL = string.Format("INSERT INTO D_StorageOutFactory  (ID ,MainID,LineNumber,SalesOrder,Style,Material,Color,Quantity,Measure) Select ID,MainID,LineNumber,SalesOrder,Style,Material,Color,Quantity,Measure  from D_StorageOutDetail Where MainID = '{0}' ",ID);
            ...
            {
                Msg.Error(string.Format("发货通知单复核出错！具体的错误信息如下：%n {0}", ex.Message));
                return false;
            }
